...
class Event1way_neighbor(Base):
    def __init__(self, Delta, alpha=1):
        super().__init__()
        self.Delta = Delta
        self.alpha = alpha

    def _to_1way_marginal(self, ground_truth, dp_data):
        ground_truth = ground_truth.groupby('neighborhood').sum().sum(axis=1)
        dp_data = dp_data.groupby('neighborhood').sum().sum(axis=1)
        return ground_truth, dp_data

    def compute_AEMC(self, ground_truth, dp_data):
        ground_truth, dp_data = self._to_1way_marginal(ground_truth, dp_data)
        num_neighbor = 278
        total_cells = num_neighbor
        num_flow_variables = total_cells * 2 + 2 + 1 # source, sink, dummy
        Delta = self.Delta
        alpha = self.alpha
        ...
        for i in range(total_cells):
            ...
            add_edge(i, dummy, 0, inf, 0.5)
            add_edge(dummy, i, 0, inf, 0.5)
